...
var arr: [Any] = .init(arrayLiteral: 1, 2, 4)
...
print((arr[4] as? Double), fourthObject)
...
@MainActor func update(productUUID: String, quantity: Int) {
    print(cartItems)
...
@MainActor func updateDict(productUUID: String, quantity: Int) {
    cartItemsDict[productUUID]?.1 = quantity
}

for kv in cartItemsDict {
...
if let gemStone = stone.polish() {
    let polishedGemstone = gemStone.polish()
}
